...
app.get("/podcasts", (req, res, next) => {
  const options = {
      ...
      body: JSON.stringify({
        query: `query maQuery{podcasts(searchTerm: "${req.query.name}") {data {title,id,imageUrl}}}`
      })
    };
    fetch("https://api.podchaser.com/graphql", options).then((response) => response.json()).then((json) => {
      res.status(200).send(JSON.stringify(json.data.podcasts.data))
    })
})
...
app.get("/episodes", (req, res, next) => {
  const options = {
    ...
    body: JSON.stringify({
      query: `query maQuery{podcast(identifier: {id: "${req.query.podcastId}", type: PODCHASER}) {title,description,imageUrl,episodes(sort: {sortBy: AIR_DATE}) {data {airDate, title, audioUrl}}}}`
    })
  };
  fetch("https://api.podchaser.com/graphql", options).then((response) => response.json()).then((json) => {
    console.log(json);
